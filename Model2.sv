...
always_comb begin
	...
	// R292: THE TEXEL FETCH HAS ITS OWN PORT AGAIN, AND THE PICTURE IS WHY.
	//
	// R290 shared port 3 between the glyph cache and the texel fetch, on the
	// reasoning that the cache's port is idle 89% of the time. The board:
	// scanline overruns went to a MEDIAN of 33 and a MAXIMUM OF 277 of 384,
	// and the tilemap came apart into horizontal tearing across the whole
	// screen. The averages were fine; the BURSTS were not, and the tile fetch
	// is the one consumer here with a hard per-scanline deadline.
	//
	// The sharing was a workaround for a hang that turned out to be R283's
...
// R248: THE TEXTURE-RAM SWEEP NEEDS THE WRITE PORT, AND NEVER HAD IT. This
// gates the boot writer's request into the arbiter, and it covered only the
// read-latency calibration's own states, 1 to 8. R223's texture-RAM zero sweep
// runs in states 12 to 15 and so requested a port it was never granted: the
// arbiter only acknowledges a port it has picked, and it only picks one that is
// requesting, so `wr_ack_st` never arrived, state 13 waited for it forever, and
// not one word of texture RAM was ever written on hardware. Nothing noticed,
// because `cal_done` is `st_state >= 12` and was already satisfied.
...
always_ff @(posedge clk_sys or negedge mem_rst_n) begin
	if (!mem_rst_n) begin
		...
		case (st_state)
			...
			4'd14: begin st_req <= 1'b0; st_state <= 4'd14; end   // done; cal_mask holds
...
wire [15:0] r3d_fillpass;        // R455: quads handed to the fill, per frame
...
m2_geometry u_geometry (
	...
	.dbg_lum_go(geo_lum_go), .dbg_lum(geo_lum),
...
// R495: R487's missed-scanline instrument is UNWIRED, not deleted. Five builds
// carrying it have failed to run where two of the four before it did, and while
// that is p ~ 0.17 and not evidence, this is the largest thing added since the
// last build that worked and it is an INSTRUMENT -- the cheapest possible thing
// to give up while the span overlap is the change actually under test. Leaving
// r3d_missed unread returns it to the 112 dead debug signals the fitter already
// prunes, taking the counter, the subtract and their routing with it. Restore
// by putting `ms_f` back in the z channel below; the RTL side never changed.

...
| **GEOMETRY ARITHMETIC — COPIED, `rtl/video/m2_fp_pool.sv`, `m2_geo_xform.sv`, `sim/video/geo_wrappers.sv`** | `rtl/video/m1_fp_pool.sv`, `m1_geo_xform.sv`, `sim/video/geo_wrappers.sv` @ **`78481c4`**, **incremental branch** | Renamed only; benches taken too. **This REFINES the "geometry does not transfer" line below — see study R170.** The arithmetic stages transfer because a 3x4 matrix transform of a 3D point is the same operation on both boards; what does not transfer is the display-list GRAMMAR, which stays ours in `m2_geo.sv`. Ported at 7,813 checks / 0 fails and **19.0 cycles per transform** streamed. Replaces a hand-rolled version written here first, which gave the stage private FP units and idled three stages of a four-stage pipeline. |

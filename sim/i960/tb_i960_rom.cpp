...
static void io_board_step() {
  ...
  if (!io_status_done && vblanks >= io_status_frame) {
    io_dp[0x21] = 0x40;
    // THE BOARD SUPPLIES THE WHOLE 128-BYTE BLOCK. The i960 copies DPRAM
    // 0x100-0x17f into backup SRAM and will not go on until it has:
    //   00228230 lda 0x1c00200,g6 / 00228238 lda 0x1d00000,g5
    //   0022827C ldob (g6),g4     / 00228280 stob g4,(g5)
    // DPRAM is the SOURCE. This model previously filled only the tail, on the
    // reading that the i960 wrote the block and the board completed it -- the
    // window's contents matched backup SRAM, and the arrow was taken backwards.
    // They match because the i960 had just copied them that way.
    static const uint8_t blk[128] = {
      ...
      0x02,0x02,0x14,0x1c, 0x00,0x01,0x00,0x01, 0x04,0x01,0xff,0xff,
